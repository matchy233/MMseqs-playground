...
int printprofile(int argc, const char **argv, const Command &command)
{
    ...
    DBReader<unsigned int> seqReader(par.db2.c_str(), par.db2Index.c_str(), par.threads, DBReader<unsigned int>::USE_INDEX | DBReader<unsigned int>::USE_DATA);
...
#pragma omp for schedule(static)
        for (size_t i = 0; i < reader.getSize(); i++)
        {
            ...
            for (size_t i = 0; i < (querySeqLen) * Sequence::PROFILE_READIN_SIZE; i += Sequence::PROFILE_READIN_SIZE) {
                seqFromWhileLoop.append(1, subMat->num2aa[(int)seqData[i + Sequence::PROFILE_AA_SIZE]]);
            }
            ...
            Debug(Debug::ERROR) << "Error: sequence length does not match for sequence " << i << " with queryKey " << queryKey << ".\n";
                Debug(Debug::ERROR) << "Expected equence length: " << querySeqLen << "\n";
                Debug(Debug::ERROR) << "Actual sequence length: " << seqFromNumSeq.length() << "\n";
                EXIT(EXIT_FAILURE);
            }
